...
STFile <- "Teaching_Hours.csv"
ST <- read.csv(STFile)
colnames(ST) <- c("CTR", "INDICATOR", "SUBJECT", "MEASURE", "FREQUENCY", "TIME", "Value", "Flag Codes")

# TEACHING HOURS #

HOURS <- sqldf("select CTR, Value from ST where FREQUENCY=='A' and TIME=='2012'")

STM <- merge(HOURS,PISAMFilt,by = "CTR")			# 37 entries
STM <- na.omit(STM)							# 32 entries
colnames(STM) <- c("LOCATION", "Teaching Hours", "PISA Maths Score")

STS <- merge(HOURS,PISASFilt,by = "CTR")			# 37 entries
STS <- na.omit(STS)							# 32 entries
colnames(STS) <- c("CTR", "Teaching Hours", "PISA Sciences Score")

# Discrete indicators and Fisher tests #

Discrete_ST_M8 <- discretize2d(STM$"Teaching Hours", STM$"PISA Maths Score", numBins1=4, numBins2=4)
Discrete_ST_M12 <- discretize2d(STM$"Teaching Hours", STM$"PISA Maths Score", numBins1=4, numBins2=4)
Discrete_ST_S8 <- discretize2d(STS$"Teaching Hours", STS$"PISA Sciences Score", numBins1=4, numBins2=4)
Discrete_ST_S12 <- discretize2d(STS$"Teaching Hours", STS$"PISA Sciences Score", numBins1=4, numBins2=4)
...
plot(STM$"Teaching Hours", STM$"PISA Maths Score")

# Plot of PISAScience = f(S/T) #
plot(STS$"Teaching Hours", STS$"PISA Sciences Score")
